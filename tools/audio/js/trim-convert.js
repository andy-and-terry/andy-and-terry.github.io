(function () {
  ...
  function redrawWaveform() {
    ...
    drawWaveform(waveformCanvas, audioBuffer, {
      selStart: parseFloat(startRange.value),
      selEnd: parseFloat(endRange.value),
    });
  }
  ...
  function handleFile(file) {
    ...
    outputName.value = baseName(file.name) + "-trimmed." + formatSelect.value;
    ...
    decodeAudioBuffer(file)
      ...
      .catch(function () {
        waveformBox.error("Waveform preview unavailable for this file — trimming still works.");
      });
  ...
  processBtn.addEventListener("click", async function () {
    ...
    try {
      ...
      const args = ["-i", inName, "-ss", start.toFixed(3), "-to", end.toFixed(3), "-vn", "-c:a", CODECS[format]];
      if (HAS_BITRATE[format]) args.push("-b:a", bitrateSelect.value);
      args.push(outName);

      setStatus("Trimming & converting…", true);
