(function () {
  ...
  const EXT_TO_FORMAT = { mp3: "mp3", wav: "wav", wave: "wav", ogg: "ogg", oga: "ogg", m4a: "aac", aac: "aac", flac: "flac" };
  ...
  function handleFile(file) {
    ...
    const guessed = EXT_TO_FORMAT[extName(file.name)] || "mp3";
    formatSelect.value = guessed;
    syncFormatUI();
    outputName.value = baseName(file.name) + "-normalized." + formatSelect.value;
    ...
    decodeAudioBuffer(file)
      ...
      .catch(function () {
        waveformBox.error("Waveform preview unavailable for this file — normalizing still works.");
      });
  }

  function updateTargetPill() {
    targetPill.textContent = targetRange.value + " LUFS";
  }
  targetRange.addEventListener("input", updateTargetPill);
  updateTargetPill();

  document.querySelectorAll("[data-target]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      targetRange.value = btn.getAttribute("data-target");
      updateTargetPill();
    });
  });
  ...
  processBtn.addEventListener("click", async function () {
    ...
    const target = parseInt(targetRange.value, 10);
    ...
    try {
      ...
      const args = ["-i", inName, "-af", "loudnorm=I=" + target + ":TP=-1.5:LRA=11", "-c:a", CODECS[format]];
      if (HAS_BITRATE[format]) args.push("-b:a", bitrateSelect.value);
      args.push(outName);

      setStatus("Analyzing & normalizing loudness…", true);
