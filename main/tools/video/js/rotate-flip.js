(function () {
  ...
  function handleFile(file) {
    ...
    outputName.value = baseName(file.name) + "-rotated.mp4";
  ...
  processBtn.addEventListener("click", async function () {
    ...
    const filterParts = buildFilter();
    if (filterParts.length === 0) {
      showError("Choose a rotation or flip first — nothing to change.");
      return;
    }
    clearError();
    resetOutput();
