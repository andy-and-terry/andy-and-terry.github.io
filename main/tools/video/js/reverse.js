(function () {
  ...
  function handleFile(file) {
    ...
    outputName.value = baseName(file.name) + "-reversed.mp4";
    ...
    preview.onloadedmetadata = function () {
      if ((preview.duration || 0) > 60) longWarning.style.display = "block";
    };
  }
  ...
  processBtn.addEventListener("click", async function () {
    ...
    function buildArgs(withAudio) {
      const args = ["-i", inName, "-vf", "reverse", "-c:v", "libx264", "-preset", "veryfast", "-crf", "23"];
      if (withAudio) args.push("-af", "areverse", "-c:a", "aac");
      else args.push("-an");
    ...
    try {
      ...
      setStatus("Reversing… this can take a while", true);
      const wantAudio = reverseAudio.checked;
